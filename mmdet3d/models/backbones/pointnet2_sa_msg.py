...
class PointNet2SAMSG(BasePointNet):
    ...
    def __init__(self,
        ...
        for sa_index in range(self.num_sa):
            ...
            csam_channels.append(cur_aggregation_channel)
        self.csam = CSAM(skip_channel_list[1:])
    ...
    def forward(self, points: Tensor):
        ...
        # features torch.Size([2, 1, 16384])
        # xyz torch.Size([2, 16384, 3])
        ...
        out_sa_xyz = [xyz] #torch.Size([2, 16384, 3])
        out_sa_features = [features] #torch.Size([2, 1, 16384])
        out_sa_indices = [indices]

        for i in range(self.num_sa):
            cur_xyz, cur_features, cur_indices = self.SA_modules[i](
                sa_xyz[i], sa_features[i])
            #cur features : torch.Size([2, 96, 4096])
        ...
        torch.Size([2, 96, 4096])
        torch.Size([2, 256, 1024])
        torch.Size([2, 512, 256])
        torch.Size([2, 1024, 64])
        '''
        sa_feats = out_sa_features[1:]
        new_feats = []
        for i, f in enumerate(sa_feats):
            B, C, N = f.shape
            H = W = int(math.sqrt(N))
            f = f.view(B, C, H, W)
            new_feats.append(f)

        o = self.CSAM(new_feats)
        # for i in o:
        #     print(i.shape)

        for i, f in enumerate(o):
            B, C, H, W = f.shape
            sa_feats[i] = f.view(B, C, -1)
            # print(sa_feats[i].shape)
        out_sa_features[1:] = sa_feats
        return dict(
            sa_xyz=out_sa_xyz,
