...
from .aatt import AreaAttention
...
class PointRCNNRoIHead(Base3DRoIHead):
    ...
    def __init__(self,
        ...
        self.att = AreaAttention(dim=130, num_heads=5, area=1)
    ...
    def _bbox_forward(self, features: Tensor, points: Tensor, batch_size: int,
        ...
        H=W = int(math.sqrt(N))
        tmp_feat = features.view(B,H,W,C).permute(0,3,1,2).contiguous()

        tmp_feat = F.interpolate(tmp_feat,scale_factor=0.5,mode='bilinear',align_corners=True)
        tmp_feat = self.att(tmp_feat)
        tmp_feat = F.interpolate(tmp_feat, scale_factor=2, mode='bilinear', align_corners=True)
        tmp_feat = tmp_feat.view(B,C,-1).permute(0,2,1).contiguous()
        assert features.shape == tmp_feat.shape
        features = tmp_feat + features
